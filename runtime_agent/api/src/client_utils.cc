...
size_t client_utils::compute_cobj_size( const MPI_ComputeObj * cobj)
{
  ushort    nargs = cobj->get_nargs();
  uint   *  types = cobj->get_types();
  size_t *  lens  = cobj->get_lengths();
  size_t    bytes = sizeof(ushort) +      /*space for number of args*/
                    nargs*sizeof(uint) +  /*space for types*/
                    nargs*sizeof(size_t); /*space for lengths*/

  for(int i=0; i < nargs; i++ )
  {
    int iBytes =0;
    MPI_Type_size(types[i], &iBytes);
    bytes += lens[i] * iBytes;
  }

  std::cout << "Final size is : " << bytes << "B" << std::endl;
  return bytes;
}


std::unique_ptr<void, client_utils::cobj_deleter_t>
client_utils::serialize_cobj( const MPI_ComputeObj * cobj)
{
  ushort    nargs = cobj->get_nargs();
  uint   *  types = cobj->get_types();
  size_t *  lens  = cobj->get_lengths();
  void  **  data  = cobj->get_data();
  size_t bytes = compute_cobj_size( cobj );
  decltype( serialize_cobj(nullptr) ) out( malloc( bytes ), cobj_deleter);

  char * barray = (char *) out.get();
  ////////////////////////////////////////////////////////////////////////
  //push nargs in;
  memcpy( barray, &nargs, sizeof( ushort ) );
  barray += sizeof( ushort );
  ////////////////////////////////////////////////////////////////////////
  //pushing types
  for(int i=0; i < nargs; i++) 
  {
    memcpy( barray, (void *) &types[i], sizeof( uint ) );
    barray += sizeof( uint );
  }
  ////////////////////////////////////////////////////////////////////////
  //pushing lengths
  for(int i=0; i < nargs; i++) 
  {
    memcpy( barray, (void *) &lens[i], sizeof( size_t ) );
    barray += sizeof( size_t );
  }
  ////////////////////////////////////////////////////////////////////////
  //pushing lengths
  for(int i=0; i < nargs; i++) 
  {
    int iBytes =0;
    MPI_Type_size(types[i], &iBytes);
    memcpy( barray, data[i], lens[i]*iBytes );
    barray += lens[i]*iBytes;
  }
  ////////////////////////////////////////////////////////////////////////
